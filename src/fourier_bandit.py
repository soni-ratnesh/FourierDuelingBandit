...
Exploits Fourier-sparse structure of preference matrices to achieve 
O(k log N) sample complexity instead of O(N²) for standard approaches.

Key idea: If the preference matrix P has a k-sparse Fourier representation,
we can recover P using compressed sensing with far fewer samples.
"""
...
class FourierDuelingBandit:
    """
    Fourier-based dueling bandit algorithm for Copeland winner identification.
    
    Exploits Fourier-sparse structure using L1-regularized regression 
    (LASSO) to recover the sparse representation from pairwise comparisons.
    
    Complexity: O(k · poly(log N)) comparisons
    """
    
    def __init__(self, N: int, z: int, k: int, X: np.ndarray, 
                 delta: float = 0.1, lambda_reg: float = None):
        """
        Parameters
        ----------
        N : int
        ...
        lambda_reg : float, optional
            ...
            If None, automatically set based on k (smaller k = stronger regularization)
        """
        self.N = N
        self.z = z
        self.k = k
        ...
        # Set lambda adaptively based on k if not provided
        # Smaller k means we expect sparser solution, so use stronger regularization
        if lambda_reg is None:
            # lambda ~ 1/k ensures stronger sparsity for smaller k
            self.lambda_reg = 0.5 / max(k, 1)
        else:
    ...
    def select_pair_informative(self) -> Tuple[int, int]:
        ...
        Prioritizes pairs with large feature differences in the top-k weighted dimensions.
        """
        # Get top-k features by absolute weight
        abs_weights = np.abs(self.w_hat)
        if np.max(abs_weights) < 0.001:
            # No weights estimated yet, use all features
            relevant = np.ones(self.z, dtype=bool)
        else:
            # Select top-k features (using self.k)
            top_k_indices = np.argsort(abs_weights)[-self.k:]
            relevant = np.zeros(self.z, dtype=bool)
            relevant[top_k_indices] = True
        ...
        for _ in range(n_candidates):
            ...
            # Weight by estimated importance, focus on top-k features
            info = np.sum(diff[relevant] * (abs_weights[relevant] + 0.1))
...
if __name__ == "__main__":
    ...
    print("Fourier Dueling Bandit Demo")
    print("=" * 50)
    ...
    # Run algorithm
    algo = FourierDuelingBandit(N=8, z=15, k=3, X=X, delta=0.1)
